...
import re,os,sys,smtplib,requests,itertools,json,random
...
def basic_termux():
	...
	slp(1)
	menu()
#__________[ WITHOUT LOGIN DUMP ]_________>>
def without_login_dump():
	banner()
	print("[01] Email Dump")
	print("[02] Random Username Dump")
	print("[03] OLD Random UID Dump")
	print("[04] Mobile Number Dump")
	print("[05] NEW Random UID Dump")
	print("[06] ")
	print("-------------------------------------------------")
	exb = input("[>>] Input an Option : ")
	if exb in ["1","01"]:
		mail_dump()
	elif exb in ["2","02"]:
		username()
		...
		slp(1)
		without_login_dump()
#__________[ MAIL_DUMP ]_________>>
def mail_dump():
	banner()
	firstname = input("[+] Input Mail Firstname : ")
	lastname = input("[+] Input Mail Lastname : ")
	domain = input("[+] Input Mail Domain : ")
	filepath = input("\n[+] Dump Mails File Save As : ")
	try:
		file = open(filepath,"w")
	except:
		file = open("dump/email.txt","w")
		filepath = "dump/email.txt"
	try:
		limit = int(input("\n[+] Input Dump Limit : "))
	except:
		limit = 5000
	...
	for i in range(limit):
		...
		if ercx == 1:
			...
			randintX = rr(999,9999)
		mail = str((firstname.lower())+str(lastname.lower())+str(randintX)+str(domain)+"|"+str(firstname)+" "+str(lastname))
		file.write(str(mail)+"\n")
	file.close()
	print("--------------------------------------------------")
	print(f"[+] Total Dump : {str(limit)}")
	print(f"[+] Dump File : {str(filepath)}")
	print("--------------------------------------------------")
	input("[+] Enter for main menu - G O D ")
	slp(1)
	menu()
#_________[ USERNAME_DUMP ]_________>>
def username():
	banner()
	firstname = input("[+] Input Mail Firstname : ")
	lastname = input("[+] Input Mail Lastname : ")
	...
	try:
		...
		file = open("dump/usernme.txt","w")
		filepath = "dump/username.txt"
	try:
		limit = int(input("\n[+] Input Dump Limit : "))
	except:
		limit = 5000
	...
	print("--------------------------------------------------")
	for i in range(limit):
		username = str(firstname.lower())+"."+str(lastname.lower())+"."+str(i)+"|"+str(firstname)+" "+str(lastname)
	...
	slp(1)
	menu()
#________[ OLD_UID_DUMP ]______>>
def new_uid_dump():
	banner()
	old_uidX = input("[+] Input NEW UID : ")
	filepath = input("\n[+] Dump File Save As : ")
	try:
		file = open(filepath,"w")
	except:
		file = open("dump/new_uid_dump.txt","w")
		filepath = "dump/new_uid.txt"
	try:
		limit = int(input("\n[+] Input Dump Limit : "))
	except:
		limit = 5000
...
def old_uid_dump():
	banner()
	old_uidX = input("[+] Input OlD UID : ")
	filepath = input("\n[+] Dump File Save As : ")
	try:
		file = open(filepath,"w")
	except:
		file = open("dump/old_uid.txt","w")
		filepath = "dump/old_uid.txt"
	try:
		limit = int(input("\n[+] Input Dump Limit : "))
	except:
		limit = 5000
...
def number():
	banner()
	print(f"Example - {green}PAK{white}: +923453927212")
	print(f"Example - {green}IND{white}: +919560507594")
	print(f"Example - {green}BAN{white}: +8801317615510")
	print("-------------------------------------------------")
	number = input("\n[+] Input number : ")
	try:
		...
		limit = 5000
	code_XYZ = (number[0:(len(str(number))-7)])
	filepath = input("[+] Input file name : ")
	try:
		file = open(filepath,"w")
	except:
		file = open("dump/number.txt","w")
		filepath = "dump/number.txt"
	print("\n--------------------------------------------------")
	print(f"          {blue}Process In Started Please Wait{white}")
	print("--------------------------------------------------")
	for i in range(limit):
		number_XYZ = str(code_XYZ)+str(rint(1111111,9999999))+"|ID"
		file.write(number_XYZ)
	file.close()
	print("--------------------------------------------------")
	print(f"[+] Total Dump : {str(limit)}")
	print(f"[+] Dump File : {str(filepath)}")
